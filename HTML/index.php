...
# I use an url variable to set and unset readonly mode (e.g. http://url/?VariableName=ReadOnly.
# Set the following to whatever you'd like (e.g. Password == Secret)

if ($_GET['VariableName'] == 'ReadOnly') {
...
global $miner, $port, $readonly, $notify, $pools, $config, $rigs, $socktimeoutsec;
...
$hidefields = array(
				'PGA.Hardware Errors' => 1,
				'PGA.Frequency' => 1,
				'PGA.Total MH' => 1,
				'CPU.Diff1 Work' => 1,
				'CPU.Difficulty Accepted' => 1,
				'CPU.Difficulty Rejected' => 1,
				'CPU.Last Share Difficulty' =>1,
				'SUMMARY.Hardware Errors' => 1,
				'SUMMARY.Total MH' => 1,
				'SUMMARY.Local Work' => 1,
				'SUMMARY.Algorithm' => 1,
				'SUMMARY.Utility' => 1,
				'SUMMARY.Difficulty Accepted' => 1,
				'SUMMARY.Difficulty Rejected' => 1,
				'SUMMARY.Difficulty Stale' => 1,
				'SUMMARY.Best Share' => 1,
				'POOL.User' => 1
				);
...
$tablebegin = '<tr><td><table cellpadding=5 cellspacing=0>';
...
<?php if ($readonly === false && $checkapi === true) { ?>
...
<table cellpadding=20 cellspacing=0 summary='Mine'>
<tr>
	<td align=center valign=top>
		<table cellpadding=0 cellspacing=0 summary='Mine'>
<?php
}
# END htmlhead()
...
function showhead($cmd, $item, $values) {
	...
	foreach ($values as $name => $value) {
		...
		echo "<td valign=middle class=h>$name</td>";
 	}
...
function details($cmd, $list, $rig) {
	global $tablebegin, $tableend, $dfmt, $miner;
	global $poolcmd, $readonly;
	global $showndate;
	...
	if ($showndate === false) {
		echo '<tr><td class=sta>'.date($dfmt).'</td></tr>';
	...
	if (isset($list['STATUS'])) {
		...
		if ($stas[$sta] === 'Success') {
			echo '<td class=okay> </td>';
		}
		else {
			echo '<td class=bad> </td>';
		}
		echo '<td>'.$miner.' - '.$list['STATUS']['Msg'].'</td>';
		echo '</tr>';
	}
...
function doforeach($cmd, $des, $sum, $head, $datetime) {
	...
	if ($datetime) {
		echo '<tr><td class=sta>'.$miner.' - '.date($dfmt).'</td></tr>';
		echo $tableend.$tablebegin;
...
function doOne($rig, $preprocess) {
	...
	global $error, $readonly, $notify, $pools, $config;
	global $rigs;
...
//	echo "<td><input type=button value='Refresh' onclick='pr(\"?rig=$rig\",null)'></td>";
